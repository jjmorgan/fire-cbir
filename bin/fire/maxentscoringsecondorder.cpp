...
/** read a lambda file trained with ME-first for using in this
    Scoring function */
void MaxEntSecondOrderScoring::load(const ::std::string &filename) {
 ...
 */
double MaxEntSecondOrderScoring::getScore(const ::std::vector<double>& dists) {
  double Z=0;
  double pc;
  double max=-numeric_limits<double>::max(); uint best=0;
  uint D=dists.size();

  vector<double> dist2nd(D*(D+1)/2+1);
  uint cnt=0;
  for(uint i=0;i<D;++i) {
    for(uint j=i;j<D;++j) {
...
const ::std::string MaxEntSecondOrderScoring::settings() {
  ostringstream oss;
