...
#ifndef __maxentsecondorderscoring_hpp__
#define __maxentsecondorderscoring_hpp__
#include "diag.hpp"
#include "basescoring.hpp"


class MaxEntSecondOrderScoring : public BaseScoring {
private:
  ::std::vector<double> lambdas_;
  double factor_, offset_;
  uint numLambdasCls_, numLambdasCom_, numLambdasPool_, numCls_;  
public:
  MaxEntSecondOrderScoring(uint no=0) : lambdas_(no) {type_="maxent2nd";}
  MaxEntSecondOrderScoring(const ::std::string &filename) {type_="maxent2nd";load(filename);}
