...
public:
  ...
  VectorFeature::VectorFeature() : data_() {type_=FT_VEC;}
  
  /// initialize a vector feature, prepared to take "size" entries
  VectorFeature::VectorFeature(uint size) : data_(size) {type_=FT_VEC;}
  
  /// take a vector and make this vector feature contain this data
  VectorFeature::VectorFeature(const ::std::vector<double> &in) : data_(in) {type_=FT_VEC;}
