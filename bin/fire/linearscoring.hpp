...
public:
  ...
  LinearScoring(uint no=0) : weights_(no) {type_="linear";}
  LinearScoring(const ::std::string& filename)  {type_="linear"; load(filename);}
    ...
    if(!is.good()) {
      ...
      if(token!="nofweights") {
        ERR << "not a valid weights file" << ::std::endl
            << " got '" << token << "' instead of 'nofweights'." << ::std::endl;
        exit(10);
      } else {
        iss >> N;
      }
