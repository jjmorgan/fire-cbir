...
const int SIFT::MIN_PATCH_SIZE = 5;          
const int SIFT::MAX_PATCH_SIZE = 101;        
const int SIFT::DEFAULT_PATCH_SIZE = 11;     
const double SIFT::INIT_SIGMA = 0.6; 
const double SIFT::MAGNITUDE_THRESHOLD = 0.02;
...
SIFT::SIFT(const ImageFeature& image) {
  img = image;
}

#ifdef HAVE_FFT_LIBRARY

void SIFT::fftwImage(const ImageFeature& img, int& paddedSize, fftw_complex* &hsTransformed, fftw_complex* &vTransformed, 
		     bool fixedSize) {
...
ImageFeature SIFT::makeFilter(double sigma) {
...
void SIFT::convolution(ImageFeature& image, double sigma) {
...
void SIFT::applyFilter(fftw_complex* &hsTransformedImage, fftw_complex* &vTransformedImage,
...
ImageFeature SIFT::fftwBackImage(fftw_complex* &hsTransformedImage, fftw_complex* &vTransformedImage, 
...
ImageFeature SIFT::difference(const ImageFeature& img1, const ImageFeature& img2) {
...
vector<InterestPoint> SIFT::getInterestPoints(int numPoints) {
#ifdef HAVE_FFT_LIBRARY
  ...
  for (int x = 1; x < (int) img.xsize() - 1; x++) {
	...
	if (isExtremum(diffImages[i - 1], diffImages[i], diffImages[i + 1], x, y)) {
	  if ((x - sizes[i] / 2 >= 0) && (int(img.xsize()) - x > sizes[i] / 2) && (y - sizes[i] / 2 >= 0) && (int(img.ysize()) - y > sizes[i] / 2)) {
	    ...
	    if (extrValue > MAGNITUDE_THRESHOLD) { // skip extrema with too low magnitude 
	      InterestPoint ip;
	      ip.x = x; 
	      ip.y = y;
	      ip.scale = sizes[i];
  ...
  if ((int) interestPoints.size() > numPoints) {
    ...
    // skip elements at the end
    interestPoints.resize(numPoints);
  }
