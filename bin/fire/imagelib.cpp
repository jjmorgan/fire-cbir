...
ImageFeature affineTransformation(const ImageFeature &image, const AffineTransformation& at) {
  ...
  for(uint i=0;i<image.ysize();++i) {
    for(uint j=0;j<image.xsize();++j) {
      y=inverted[0][0]*(i-at.b[0])+inverted[0][1]*(j-at.b[0]);
      x=inverted[1][0]*(i-at.b[1])+inverted[1][1]*(j-at.b[1]);
...
void rect(ImageFeature &img, const uint x, const uint y, const uint w, const uint h, const ::std::vector<double> &color) {
  for(uint i=x;i<x+w;++i) { for(uint c=0;c<img.zsize();++c) {img(i,y,c)=color[c];img(i,y+h,c)=color[c];}}
  for(uint i=y;i<y+h;++i) { for(uint c=0;c<img.zsize();++c) {img(x,i,c)=color[c];img(x+w,i,c)=color[c];}}
}
