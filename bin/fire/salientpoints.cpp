...
vector<Point> SalientPoints::sPoints(const string& wavelet, float seuil, vector<float>& floatPixels, int bord, bool display) {
  int l, c, i;
  int nb = 0;
  int nb_total = 0;
...
vector<Point> SalientPoints::getSalientPoints(int nrPoints) {
  ...
  //    cout << floatPixels[i] << " ";
  //  }cout << endl;
  float seuil = float(nrPoints);
  
  struct timeb timeNow, timeLastChanged;
  int elapsedTime, second, milliSecond;
  ftime (&timeLastChanged);
  
  vector<Point> pointVector=sPoints("daub4", seuil, floatPixels, BORD, true);
