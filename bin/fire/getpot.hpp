...
inline void
GetPot::set(const char* VarName, const double& Value, const bool /* = yes */)
{ __set_variable(VarName, __double2string(Value).c_str()); }

inline void 
GetPot::set(const char* VarName, const int Value, const bool /* = yes */)
...
GetPot::__DBE_expand(const std::string expr)
{
	...
	while( (tmp = A[0].find(A[1])) != A[0].size() ) {
